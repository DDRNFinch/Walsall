(() => {
  ...
  function syncACSFromScreen(p, e) {
    e.acsResponses = e.acsResponses || {};
    const boxes = document.querySelectorAll('.acs-response');
    boxes.forEach(box => {
      const code = box.closest('.acs-box')?.querySelector('.ksb-code')?.textContent?.trim();
      if (code) e.acsResponses[code] = box.value || '';
    });
    state.evidence[p.id] = e;
    save();
    return e;
  }
  ...
  function draw6570PhotoPage(doc, p, photos) {
    ...
    const count = p.capture.length, cols = 2, rows = Math.ceil(count / cols), gap = 4;
    const gridTop = PDF.top + 22, gridBottom = PDF.pageH - PDF.bottom - 8;
    const cellW = (PDF.width - gap) / cols, cellH = (gridBottom - gridTop - gap * (rows - 1)) / rows;
    for (let n = 0; n < count; n++) {
      const col = n % cols, row = Math.floor(n / cols);
      const x = PDF.left + col * (cellW + gap), y = gridTop + row * (cellH + gap);
      doc.setDrawColor(190); doc.rect(x, y, cellW, cellH);
  ...
  function addACSResponsesPage(doc, p, e) {
    ...
    doc.addPage();
    const L = PDF.left, W = PDF.width, bottom = PDF.pageH - PDF.bottom - 5;
    let y = PDF.top + 3;
    const header = () => {
      doc.setFont('helvetica','bold'); doc.setFontSize(16); doc.text('ACS Responses', L, y); y += 8;
      doc.setFont('helvetica','normal'); doc.setFontSize(8);
      y = writeWrapped(doc, `${p.title} · Unit ${p.unitId} · Learner: ${state.profile.name || 'Not entered'}`, L, y, W, 3.5) + 5;
    };
    header();
    ...
    for (const item of completed) {
      ...
      if (y + needed > bottom) {
        footer(doc, doc.getNumberOfPages()); doc.addPage(); y = PDF.top + 3; header();
      }
      doc.setFont('helvetica','bold'); doc.setFontSize(10); doc.text(item.code, L, y); y += 5;
      doc.setFont('helvetica','normal'); doc.setFontSize(8); y = writeWrapped(doc, item.wording, L, y, W, 3.8) + 3;
      doc.setFont('helvetica','bold'); doc.setFontSize(8); doc.text('Learner response', L, y); y += 4;
      doc.setFont('helvetica','normal'); doc.setFontSize(8); y = writeWrapped(doc, response, L, y, W, 3.8) + 7;
    }
  ...
  async function download6570(i) {
    ...
    if (!p) return;
    const e = state.evidence[p.id] || { photos: [], statement: '', signature: '', signedDate: '', tags: {} };
    syncACSFromScreen(p, e);
    ...
    if (photos.length < required || photos.some(x => !x)) {
      toast(`All ${required} photos are required before downloading`); return;
    }
    ...
    doc.setFont('helvetica','normal'); doc.setFontSize(8);
    y = writeWrapped(doc, `Learner: ${state.profile.name || 'Not entered'} | Course: ${state.profile.courseKey} | Pack: ${p.title}`, L, y, W, 3.5) + 3;
    doc.setFont('helvetica','bold'); doc.setFontSize(10); doc.text('Photo details', L, y); y += 5;
    doc.setFont('helvetica','normal'); doc.setFontSize(7.5);
    p.capture.forEach((x, n) => { y = writeWrapped(doc, `${n + 1}. ${x}`, L, y, W, 3.2) + 1; });
    y += 2;
    doc.setFont('helvetica','bold'); doc.setFontSize(10); doc.text('Learner statement', L, y); y += 5;
    doc.setFont('helvetica','normal'); doc.setFontSize(7.5); y = writeWrapped(doc, e.statement || '', L, y, W, 3.2) + 5;
    ...
    doc.setFont('helvetica','normal'); doc.setFontSize(8); y = writeWrapped(doc, acs.map(c => c.code).join(', '), L, y, W, 4) + 6;
    ...
    y += 20; doc.setFont('helvetica','normal'); doc.setFontSize(7);
    doc.text(`Signed electronically on ${e.signedDate || nowUK()}`, L, y); footer(doc, doc.getNumberOfPages());
